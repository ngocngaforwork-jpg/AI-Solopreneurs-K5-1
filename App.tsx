...
const COACH = {
  name: "Nga Nami",
  role: "Người Bạn Đồng Hành Của Bạn",
  image: "https://i.postimg.cc/2SXQTjRL/She-should-also-wear-tiny-pearl-stud-earrings-sma-delpmaspu.jpg",
  cropPos: "object-[center_25%]",
  bio: "Chào bạn, tôi là Nga Nami - Người Bạn Đồng Hành của bạn. Trong kỷ nguyên AI đầy biến động, tôi ở đây để đảm bảo bạn không bao giờ đơn độc. Tôi sẽ sát cánh, hỗ trợ và giải đáp mọi thắc mắc của bạn xuyên suốt chương trình, cho đến khi bạn thực sự làm chủ được công cụ và biến chúng thành cỗ máy vận hành cho riêng mình."
};
      ...
      <section id="coach" className="section-padding px-6 bg-apple-gray/30 border-t border-apple-gray">
        <div className="max-w-[1200px] mx-auto">
          <div className="text-center mb-12 md:mb-16 space-y-4 md:space-y-6">
             <div className="inline-flex items-center gap-2 text-apple-blue font-black text-[10px] md:text-sm tracking-widest uppercase mb-1">
              <Sparkles className="w-[18px] h-[18px] md:w-5 md:h-5" /> Dedicated Support
            </div>
            <h2 className="text-[34px] md:text-7xl font-black tracking-tighter text-apple-black uppercase">COACH ĐỒNG HÀNH.</h2>
          </div>

          <div className="max-w-4xl mx-auto">
            <div className="apple-card p-8 md:p-16 flex flex-col md:flex-row items-center gap-8 md:gap-16 border border-black/5 bg-white shadow-xl">
              <div className="relative group/img flex-shrink-0 w-48 h-48 md:w-72 md:h-72">
                <div className="absolute inset-0 bg-apple-blue rounded-full rotate-6 opacity-5 group-hover/img:rotate-0 transition-transform duration-500"></div>
                <div className="relative w-full h-full rounded-full overflow-hidden shadow-2xl ring-4 ring-white bg-[#F5F5F7]">
                  <img 
                    src={COACH.image} 
                    alt={COACH.name} 
                    className={`absolute inset-0 w-full h-full object-cover ${COACH.cropPos} grayscale hover:grayscale-0 transition-all duration-700 scale-110 hover:scale-100`}
                  />
                </div>
              </div>
              ...
              <div className="flex-1 space-y-6 text-center md:text-left">
                <div className="space-y-2">
                  <h3 className="text-3xl md:text-5xl font-black text-apple-black tracking-tighter uppercase">{COACH.name}</h3>
                  <p className="text-apple-blue font-black tracking-widest text-xs md:text-base uppercase opacity-70">{COACH.role}</p>
                </div>
                <p className="text-apple-dark-gray font-bold text-base md:text-xl leading-relaxed italic">
                  "{COACH.bio}"
                </p>
                <div className="pt-4">
                  <div className="inline-flex items-center gap-2 bg-green-50 text-green-600 px-4 py-2 rounded-full text-xs md:text-sm font-black uppercase tracking-widest">
                    <ShieldCheck size={16} /> Hỗ trợ 1:1 trong suốt chương trình
                  </div>
                </div>
              </div>
            </div>
